...
data = pd.read_csv('./server/videos/client/data.csv')
data['Time'] = data['Time'].apply(convert_to_seconds)

resolution_data = pd.read_csv('./server_over_network/videos/client/resolutionData.csv')
resolution_data['Time'] = resolution_data['Time'].apply(convert_to_seconds)

droppedFrames_data = pd.read_csv('./server_over_network/videos/client/droppedFrames.csv')
...
plt.figure()
plt.plot(droppedFrames_data['Time'], droppedFrames_data['DroppedFrames'], label='Dropped Frames')
plt.legend(loc='upper center', bbox_to_anchor=(0.5, -0.05), fancybox=True, shadow=True, ncol=5)
