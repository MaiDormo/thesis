...
data = read_and_process_data(f'statistics/mininet/{bw}_{delay}_{loss}/data_{bw}_{delay}_{loss}_{run}.csv')
resolution_data = read_and_process_data(f'statistics/mininet/{bw}_{delay}_{loss}/resolutionData_{bw}_{delay}_{loss}_{run}.csv')
resolution_data['Resolution '] = resolution_data['Resolution'].apply(lambda x: resolution_order.index(x))
droppedFrames_data = read_and_process_data(f'statistics/mininet/{bw}_{delay}_{loss}/droppedFrames_{bw}_{delay}_{loss}_{run}.csv')

data_traffic = read_and_process_data(f'statistics/mininet/traffic/{bw}_{delay}_{loss}/data_traffic_{bw}_{delay}_{loss}_{run}.csv')
resolution_data_traffic = read_and_process_data(f'statistics/mininet/traffic/{bw}_{delay}_{loss}/resolutionData_traffic_{bw}_{delay}_{loss}_{run}.csv')
resolution_data_traffic['Resolution '] = resolution_data_traffic['Resolution'].apply(lambda x: resolution_order.index(x))
droppedFrames_data_traffic = read_and_process_data(f'statistics/mininet/traffic/{bw}_{delay}_{loss}/droppedFrames_traffic_{bw}_{delay}_{loss}_{run}.csv')
...
plot_data(resolution_data, 'statistics/mininet/mininet_resolution.png', resolution_order, ['Resolution'], ax=ax1, cmap='cool', linestyle='-')
plot_data(resolution_data_traffic, 'statistics/mininet/mininet_resolution.png', resolution_order, ['Resolution'], ax=ax1, label_suffix=' (traffic)', is_traffic=True, cmap='coolwarm', linestyle='--')
