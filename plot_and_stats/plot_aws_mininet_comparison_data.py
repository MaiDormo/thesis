...
def main(params):
    ...
    data_aws = read_and_process_data(f'statistics/{params["aws"]}/data_{params["aws"]}_{params["run"]}.csv')
    resolution_data_aws = read_and_process_data(f'statistics/{params["aws"]}/resolutionData_{params["aws"]}_{params["run"]}.csv')
    resolution_data_aws['Resolution '] = resolution_data['Resolution'].apply(lambda x: params['resolution_order'].index(x))
    droppedFrames_data_aws = read_and_process_data(f'statistics/{params["aws"]}/droppedFrames_{params["aws"]}_{params["run"]}.csv')

    fig, ax = plt.subplots()
    plot_data(data, 'statistics/mininet/mininet_data.png', exclude_columns=['Effective Bitrate'], ax=ax, linestyle='-')
    plot_data(data_aws, 'statistics/mininet/mininet_data.png', exclude_columns=['Effective Bitrate'], ax=ax, label_suffix=' (aws)', is_traffic=True, linestyle='--')
    ...
    plot_data(resolution_data_aws, 'statistics/mininet/mininet_resolution.png', params['resolution_order'], ['Resolution'], ax=ax1, label_suffix=' (aws)', is_traffic=True, cmap='coolwarm', linestyle='--')
    ...
    plot_data(droppedFrames_data_aws, 'statistics/mininet/mininet_dropped_frames.png', ax=ax2, label_suffix=' (aws)', is_traffic=True, cmap='summer', linestyle=':')
